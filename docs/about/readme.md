你的实验室目前处于**初创阶段（1位导师 + 若干研究生）**，因此**不要照搬大型企业的权限体系**，否则以后维护会很麻烦。

GitHub 官方推荐采用 **Team（团队）+ Repository Role（仓库角色）** 的方式管理权限，而不是给每个人单独授权。仓库角色从低到高依次为 **Read、Triage、Write、Maintain、Admin**。([GitHub Docs][1])
